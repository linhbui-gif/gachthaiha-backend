{!! Form::open(['route' => 'admin.size.batch_process', 'method' => 'POST', 'class' => 'frm_batch_process']) !!}
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="order_by {{ $request->order_by == 'sizes.id' ? 'active' : '' }}"
                data-order-by="sizes.id" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.id') }}
                {!! $request->order_by == 'sizes.id' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th class="order_by {{ $request->order_by == 'sizes.name' ? 'active' : '' }}"
                data-order-by="sizes.name" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.name') }}
                {!! $request->order_by == 'sizes.name' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>Danh mục</th>
            <th class="order_by {{ $request->order_by == 'sizes.status' ? 'active' : '' }}"
                data-order-by="sizes.status" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.status') }}
                {!! $request->order_by == 'sizes.status' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>Hành động</th>
        </tr>
        </thead>
        ...
        @if(!empty($listData))
            @foreach($listData as $key => $value)
                <tr>
                    ...
                    <td>{{ $value->id }}</td>
                    <td>{{ $value->name }}</td>
                    <td>{{ implode(', ', $value->category->pluck('name')->toArray()) }}</td>
                    <td>
                        @if($value->status == \App\Models\BaseModel::STATUS_ACTIVE)
                            <label class="label label-success update_status"
                                   ...
                                   data-url="{{ route('admin.size.update_status') }}">
                    ...
                    <td>
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-default btn-sm btn_copy_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.size.copy', ['id' => $value->id]) }}">
                                <i class="fa fa-copy"></i>
                            </div>
                        </span>
                        <span class="help_tooltip" data-toggle="tooltip"
                              data-original-title="{{ trans('admin.edit_this_record') }}">
                            <a href="{{ route('admin.size.edit', ['id' => $value->id]) }}"
                        ...
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-danger btn-sm btn_delete_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.size.destroy', ['id' => $value->id]) }}">
