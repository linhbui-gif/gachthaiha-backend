{!! Form::open(['route' => 'admin.brand.batch_process', 'method' => 'POST', 'class' => 'frm_batch_process']) !!}
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="order_by {{ $request->order_by == 'brands.id' ? 'active' : '' }}"
                data-order-by="brands.id" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.id') }}
                {!! $request->order_by == 'brands.id' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>
                Ảnh logo
            </th>
            <th class="order_by {{ $request->order_by == 'brands.name' ? 'active' : '' }}"
                data-order-by="brands.name" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.name') }}
                {!! $request->order_by == 'brands.name' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th class="order_by {{ $request->order_by == 'brands.status' ? 'active' : '' }}"
                data-order-by="brands.status" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.status') }}
                {!! $request->order_by == 'brands.status' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>Hành động</th>
        </tr>
        </thead>
                    ...
                    <td>
                        @if($value->status == \App\Models\BaseModel::STATUS_ACTIVE)
                            <label class="label label-success update_status"
                                   ...
                                   data-url="{{ route('admin.brand.update_status') }}">
                    ...
                    <td>
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-default btn-sm btn_copy_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.brand.copy', ['id' => $value->id]) }}">
                        ...
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-danger btn-sm btn_delete_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.brand.destroy', ['id' => $value->id]) }}">
