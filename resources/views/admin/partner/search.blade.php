{!! Form::open(['route' => 'admin.partner.batch_process', 'method' => 'POST', 'class' => 'frm_batch_process']) !!}
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="order_by {{ $request->order_by == 'partners.id' ? 'active' : '' }}"
                data-order-by="partners.id" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.id') }}
                {!! $request->order_by == 'partners.id' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>
                Ảnh logo
            </th>
            <th class="order_by {{ $request->order_by == 'partners.name' ? 'active' : '' }}"
                data-order-by="partners.name" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.name') }}
                {!! $request->order_by == 'partners.name' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th class="order_by {{ $request->order_by == 'partners.status' ? 'active' : '' }}"
                data-order-by="partners.status" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.status') }}
                {!! $request->order_by == 'partners.status' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>Hành động</th>
        </tr>
        </thead>
                    ...
                    <td>
                        @if($value->status == \App\Models\BaseModel::STATUS_ACTIVE)
                            <label class="label label-success update_status"
                                   ...
                                   data-url="{{ route('admin.partner.update_status') }}">
                    ...
                    <td>
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-default btn-sm btn_copy_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.partner.copy', ['id' => $value->id]) }}">
                                <i class="fa fa-copy"></i>
                            </div>
                        </span>
                        <span class="help_tooltip" data-toggle="tooltip"
                              data-original-title="{{ trans('admin.edit_this_record') }}">
                            <a href="{{ route('admin.partner.edit', ['id' => $value->id]) }}"
                        ...
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-danger btn-sm btn_delete_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.partner.destroy', ['id' => $value->id]) }}">
