{!! Form::open(['route' => 'admin.product_type.batch_process', 'method' => 'POST', 'class' => 'frm_batch_process']) !!}
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="order_by {{ $request->order_by == 'product_types.id' ? 'active' : '' }}"
                data-order-by="product_types.id" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.id') }}
                {!! $request->order_by == 'product_types.id' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th class="order_by {{ $request->order_by == 'product_types.name' ? 'active' : '' }}"
                data-order-by="product_types.name" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.name') }}
                {!! $request->order_by == 'product_types.name' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th class="order_by {{ $request->order_by == 'product_types.status' ? 'active' : '' }}"
                data-order-by="product_types.status" data-order="{{ $request->order }}"
                data-form=".frm_form_search">{{ trans('admin.status') }}
                {!! $request->order_by == 'product_types.status' ? ($request->order == 'ASC' ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') : '<i class="fa fa-sort"></i>' !!}
            </th>
            <th>Hành động</th>
        </tr>
        </thead>
                    ...
                    <td>
                        @if($value->status == \App\Models\BaseModel::STATUS_ACTIVE)
                            <label class="label label-success update_status"
                                   ...
                                   data-url="{{ route('admin.product_type.update_status') }}">
                    ...
                    <td>
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-default btn-sm btn_copy_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.product_type.copy', ['id' => $value->id]) }}">
                        ...
                        <span class="help_tooltip" data-toggle="tooltip"
                            ...
                            <div class="btn btn-danger btn-sm btn_delete_record" data-id="{{ $value->id }}"
                                 data-url="{{ route('admin.product_type.destroy', ['id' => $value->id]) }}">
